CREATE DATABASE COMMERCIALproject;
USE COMMERCIALproject;
----------------------------------------------------------------------------------
------------------------------------------------------------------------------------
...
--------------------------------------------------------------------------------
---------------------------------------------------------------------------------------
ALTER TABLE PRODUCTS ADD CONSTRAINT FK_PRODUCTS _CATEGORIE 
FOREIGN KEY (CATEGORY_CODE) REFERENCES CATEGORIES (CATEGORY_CODE);

ALTER TABLE PRODUCTS  ADD CONSTRAINT FK_PRODUCTS _SUPPLIERS  
...
--------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------
--- 3. Display in descending order of seniority the male employees whose net salary (salary + commission) is 
...
SELECT EMPLOYEE_NUMBER, 
	   ...
	   (2024-(YEAR(HIRE_DATE))) AS SENIORITY
FROM EMPLOYEES
WHERE  SALARY > = 8000
AND TITLE IN ('Dr.','Mr.')
ORDER BY SALARY, SENIORITY DESC; 

----------------------------------------------------
-------------------------------------------------
-- 4. Display Products Meeting Criteria
--a.quantity is packaged in bottle(s)
--b. he third character in the product name is 't' or 'T',
--c. supplied by suppliers 1, 2, or 3, 
--d. unit price ranges between 70 and 200, and (C5) units ordered are specified (not null)

SELECT * FROM [dbo].[PRODUCTS]

SELECT *
FROM PRODUCTS
WHERE UNIT_PRICE BETWEEN 70.00 AND 200.00
AND PRODUCT_NAME LIKE '__t%'
AND QUANTITY LIKE '%Bottles%'
AND SUPPLIER_NUMBER BETWEEN  1 AND 3
AND UNITS_ON_ORDER IS NOT NULL;
  
  SELECT * 
  FROM PRODUCTS
  WHERE SUPPLIER_NUMBER BETWEEN  1 AND 3
...
--------------------------------------------
-----------------------------------------------
----5. Customers in the Same Region as Supplier 1
SELECT * FROM [dbo].[CUSTOMERS]
SELECT * FROM SUPPLIERS
...
WHERE s.SUPPLIER_NUMBER = 1;
 ...
 ---------------------------------------------
 ---------------------------------------------
 -- 6. New Discount Rate for Orders 10998 to 11003
...
ORDER BY NEW_DISCOUNT_RATE DESC


--------------------------------------------
--------------------------------------------
-- 7. Suppliers of Beverage Products
SELECT * FROM [dbo].[SUPPLIERS]
SELECT * FROM [dbo].[PRODUCTS]
...
WHERE SUPPLIER_NUMBER IN (
    ...
    FROM PRODUCTS
    WHERE CATEGORY_CODE = 
	(SELECT CATEGORY_CODE FROM CATEGORIES WHERE CATEGORY_NAME = 'Beverages')
);


---------------------------------------------
---------------------------------------------
-- 8. Display customers from Berlin with ≤ 1 Dessert Product Ordered
SELECT * FROM [dbo].[ORDER_DETAILS]
SELECT * FROM CUSTOMERS
SELECT * FROM [dbo].[ORDERS]
SELECT * FROM [dbo].[PRODUCTS]
...
FROM 
    CUSTOMERS C
LEFT JOIN ORDERS O1 -- Corrected the alias to match throughout the query
    ON C.CUSTOMER_CODE = O1.CUSTOMER_CODE
LEFT JOIN ORDER_DETAILS OD 
    ON O1.ORDER_NUMBER = OD.ORDER_NUMBER
LEFT JOIN PRODUCTS P 
    ON OD.PRODUCT_REF = P.PRODUCT_REF
JOIN [dbo].[CATEGORIES] G
    ON P.CATEGORY_CODE = G.CATEGORY_CODE
WHERE 
    C.CITY = 'Berlin'
GROUP BY 
    C.CUSTOMER_CODE, 
    ...
    G.CATEGORY_NAME
HAVING 
    SUM(CASE WHEN G.CATEGORY_NAME = 'Desserts' THEN 1 ELSE 0 END) <= 1;

--------------------------------------------------
--------------------------------------------------
--- 9. Customers in France and Orders Every Monday in April 1998

SELECT C.CUSTOMER_CODE,
       C.COMPANY,
       C.PHONE, C.COUNTRY,
       SUM(OD.UNIT_PRICE * OD.QUANTITY) AS 'TOTAL AMOUNT'
FROM  [dbo].[CUSTOMERS] C
LEFT JOIN ORDERS O 
...
WHERE C.COUNTRY = 'France'
  AND (O.ORDER_DATE IS NULL OR (YEAR (O.ORDER_DATE) = 1998
                                ...
                                AND DATENAME(WEEKDAY, O.ORDER_DATE) = 'MON'))
GROUP BY C.CUSTOMER_CODE, C.COMPANY, C.PHONE, C.COUNTRY;

--------------------------------------------------------
-------------------------------------------------------
-- 10. Customers Who Ordered All Products
SELECT 
C.CUSTOMER_CODE, 
C.COMPANY, 
C.PHONE
...
-------------------------------------------------
-------------------------------------------------
-- 11. Number of Orders per Customer from France
SELECT* FROM ORDERS
...
-------------------------------------------------
-------------------------------------------------
-- 12. Orders in 1996 and 1997 with Difference
SELECT SUM(CASE WHEN YEAR (ORDER_DATE) = 1996 THEN 1 ELSE 0 END) AS ORDERS_IN_1996,
	   ...
	   SUM(CASE WHEN YEAR (ORDER_DATE) = 1996 THEN 1 ELSE 0 END)  AS 'DIFF BETWEEN'
FROM 
ORDERS;
